<?php
   ...
   $id_usuario = $_GET['id_usuario'];
    if (isset($_POST['editar_rol_usuario'])) {
        $data = $_POST;
        $data['id_usuario'] = $id_usuario;
        $sitio->editar_rol_usuario($data);
        die();
    }
?>
<html>
<head>
    <title>Lynx-Space | Editar rol/usuario</title>
...
<body>
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12">
                <h1>Nuevo Rol</h1>
                <form method="POST" action="editar_rol_usuario.php" enctype="multipart/form-data">
                    <div class="form-group">
                        <label>Usuario</label>
                        <input type="text" class="form-control" placeholder="Descripcion del privilegio" name="privilegio" value="<?php echo $id_usuario ?>">
                    </div>
                    <div class="form-group">
                        <label>ID Rol</label>
                        <input type="number" class="form-control" placeholder="ID del rol" name="id_rol">
                    </div>
                    <input type="submit" name="editar_rol_usuario" value="Confirmar" class="btn btn-primary btn-lg btn-block"/>
                </form>
            	</br>
